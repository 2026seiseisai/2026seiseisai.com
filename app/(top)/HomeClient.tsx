...
function formattedNewsDate(value: string): string {
  return newsDateFormatter.format(new Date(value)).replaceAll('/', '.');
}

function NewsTeaser({ articles }: { articles: HomeNewsArticle[] }) {
  return (
    <section className="content-section news-section">
      <SectionLabel text="NEWS" />
      <div className="news-teaser-body">
        {articles.length === 0 ? (
          <p className="news-teaser-empty">
            現在公開中のお知らせはありません。
          </p>
        ) : (
          <ul className="news-teaser-list">
            {articles.map((article) => (
              <li
                key={article.id}
                className={`news-teaser-row${
                  article.important ? ' news-teaser-row-important' : ''
                }`}
              >
                <Link
                  className="news-teaser-link"
                ...
                >
                  ...
                  >
                    {formattedNewsDate(article.publishedAt)}
                  </time>
                  <span className="news-teaser-title">
                    {article.important ? (
                      <span className="visually-hidden">重要なお知らせ: </span>
                    ) : null}
                    {article.title}
                  </span>
                </Link>
        ...
        <Link href="/news" className="news-teaser-more">
          Newsをもっと見る
